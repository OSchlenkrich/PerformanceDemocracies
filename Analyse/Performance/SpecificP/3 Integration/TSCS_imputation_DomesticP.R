...
mice_data_ds = as.data.frame(tscs_data) %>%
  select_at(vars(country_text_id, year,
                 ...
                 # Economic Inequality
                 eco_inequal_num_ctl = eco_inequal_soc)) %>% 
  ...
  # Spatial Correlation %>% 
  left_join(spatial_dv_year(tscs_data, "pubsafe_ds"), by=c("country_text_id", "year")) %>% 
...
p1 = xyplot(mice_data_ds_trans, "pubsafe_ds", c("pubsafe_ds_spatial_ctl")) 
p1
