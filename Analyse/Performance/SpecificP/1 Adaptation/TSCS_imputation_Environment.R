...
mice_data_env = as.data.frame(tscs_data) %>%
  ...
  # Spatial Correlation %>% 
  left_join(spatial_dv_year(tscs_data, "air_env"), by=c("country_text_id", "year")) %>% 
  left_join(spatial_dv_year(tscs_data, "abstraction_env"), by=c("country_text_id", "year")) %>% 
...
p1 = xyplot(mice_data_env_trans, "air_env", c("air_env_spatial_ctl")) 
p2 = xyplot(mice_data_env_trans, "abstraction_env", c("abstraction_env_spatial_ctl")) 
