...
for (i in 1:2) {
  env_list[[i]]  = a.out_env$imputations[[i]] %>% 
    ...
    mutate_at(vars(starts_with("FKM_")), funs(zeroadjuster(.))) %>% 
    
    mutate(mb_sum = FKM_5_mb_fEc + FKM_5_mb_Fec + FKM_5_mb_FeC + FKM_5_mb_fEC + FKM_5_mb_FEC) %>%
    mutate_at(vars(starts_with("FKM_")), funs(./mb_sum)) %>%
    select(-mb_sum) %>%

    mutate(logFKM_5_mb_Fec_FEC = log(FKM_5_mb_Fec/FKM_5_mb_FEC),
           logFKM_5_mb_fEc_FEC = log(FKM_5_mb_fEc/FKM_5_mb_FEC),
           logFKM_5_mb_FeC_FEC = log(FKM_5_mb_FeC/FKM_5_mb_FEC),
           logFKM_5_mb_fEC_FEC = log(FKM_5_mb_fEC/FKM_5_mb_FEC)) %>% 
    ...
    mutate_at(vars(starts_with("logFKM_")), funs(wi = . - mean(., na.rm=T))) %>%
    mutate_at(vars(starts_with("logFKM_")), funs(lag = dplyr::lag(.,1))) %>% 
    mutate_at(vars(starts_with("logFKM_")), funs(df = first_DF(.)))  %>%
    
    #between effect
    mutate_at(vars(ends_with("num_ctl")), funs(bw = mean(., na.rm=T))) %>% 
    mutate_at(vars(starts_with("logFKM_")), funs(bw = mean(., na.rm=T))) %>% 

    select_at(vars(country_text_id, year_0,
                   starts_with("logFKM_5"),
                   matches("env"),
                   matches("ctl"),
                   -matches("index"))
    ) %>%
...
LogRATIOS = LR(a.out_soc$imputations[[1]] %>%  select_at(vars(starts_with("FKM_5"))) %>% 
     mutate_at(vars(starts_with("FKM_")), funs(zeroadjuster(.))))
...
for (i in 1:2) {
  soc_list[[i]]  = a.out_soc$imputations[[i]] %>% 
    ...
    # Compute LogRatios and Closure
    bind_cols(data.frame(LogRATIOS$LR)) %>% 
