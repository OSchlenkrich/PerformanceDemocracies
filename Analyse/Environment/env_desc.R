# Environment Descriptive Analyse of Factor Scores
...
env_scores_mean = env_scores %>% 
  melt(id.vars=c("country_text_id", "year")) %>% 
  group_by(country_text_id, year) %>% 
  summarise(environment_oecd_index = mean(value, na.rm=T)) %>% 
  ungroup() 
...
Environment_Performance_final = Environment_Performance %>%
  ...
  left_join(env_scores_mean, by=c("country_text_id", "year")) %>% 
  #left_join(fa_data_oecd_frame %>% select(-country), by=c("country_text_id", "year")) %>%
  left_join(fa_data_wdi_frame %>% select(-country), by=c("country_text_id", "year")) %>%
  ungroup() %>% 
