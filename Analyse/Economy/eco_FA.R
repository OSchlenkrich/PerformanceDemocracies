...
produce_fa_scores_eco = function(mice_data, nr_imputations, nr_factors, variable_ending) {
  ...
  for (i in 1:nr_imputations) {
    stack_data = mice_data %>% 
      ...
      mutate(scores = 0.4*GDP_capita_oecd_num_eco + 0.2*Inflation_oecd_num_eco + 
               0.2*Interest_oecd_num_eco + 0.2*Balance_oecd_num_eco,
             scores = scale(scores)
      )
    
    # Extract Bartlett Factor Scores
    scores_data[,i] = as.numeric(stack_data$scores)
  }
  return(scores_data)
}
...
eco_scores = fa_data_oecd_frame_mice_inv %>% 
  ...
  bind_cols(produce_fa_scores_eco(imputed_eco_inv %>% 
                                select(-"Unemployment_pr_oecd_num_eco", -"Invest_oecd_num_eco"), 10, 1, "eco"))

fa_oecd_eco_test = fa(imputed_eco_inv %>% 
                        filter(.imp==1) %>% 
                        select_at(vars(ends_with("eco"))) %>% 
                        select(-"Unemployment_pr_oecd_num_eco", -"Invest_oecd_num_eco"), 1,
