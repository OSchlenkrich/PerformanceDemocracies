...
confidence_IVS = IVS %>% 
  dplyr::select(
    ...
    # SES
    education_ind_ord_aux_ivs = X025,
    gender_ind_ord_aux_ivs = X001,
    age_ind_num_aux_ivs = X003,
    
    # Participation
    interest_ind_ord_aux_ivs = E023,

    # Culture, Opinion
    trust_ind_ord_aux_ivs = A165,
...
lambda = 0.5
confidence_IVS %>% 
  mutate_at(vars(age_ind_num_aux_ivs), funs(trans_function(.,lambda))) %>% 
  ggplot(aes(x=age_ind_num_aux_ivs)) +
  geom_histogram()

confidence_IVS_norm = confidence_IVS %>% 
  mutate_at(vars( starts_with("age_ind_num_aux_ivs")), funs(trans_function(.,lambda))) 
